...
> 캐시 메모리는 속도가 빠른 장치(CPU)와 느린 장치(주 기억장치) 간의 병목 현상을 줄이기 위한 범용 메모리이다.  
> 캐시 메모리는 메모리 계층 구조에서 가장 빠른 소자, 처리속도가 CPU에 준한다, 주 기억 장치 접근 횟수를 줄여서 처리 성느을 상승시킴  
> 캐시의 크기는 보통 kByte단위다. 캐시의 성능은 결국 CPU가 참조 할 데이터를 얼마나 잘 적중(HIT)해서 가지고 있냐에 따라 달라지고 그에 지역성(Locality)의 원리를 사용한다.

1. 지역성 : 모든 코드나 데이터에 균등하게 접근하지 않는다. 지역성이란 어느 순간 어느 특정 부분을 집중적으로 참조하는 특성이다.
2. 시간 지역성(Temporal Locality) : 최근에 참조된 주소의 내용이 곧 다음에 다시 참조되는 특성.
3. 공간 지역성(Spatial Locality) : 대부분의 실제 프로그램이 참조된 주소와 인접한 주소의 내용이 다시 참조 되는 특성
4. 캐싱 라인(Caching Line) : 캐시 메모리는 가까이 위치하며 빈번히 사용되는 데이터를 저장하는 장소이지만 찾고자 하는 데이터가 어느 곳에 저장 된지 모르면 데이터 탐색을 위해 순회해야하고 그럼 시간이 오래 걸린다. 캐쉬에 저장 된 목적 데이터에 바로 접근해 출력 할 수 있게 하기 위해서 데이터를 저장 할때 사용 하는 묶음 형태로 저장하는데 그 묶음들의 메모리 주소를 담아둔 자료 구조를 캐싱라인이라 한다.
5. 캐시 메모리의 맵핑 프로세스(캐쉬 라인)은 대표적으로 3가지 있다. 각각 Full Associative, Set Associative, Direct Map이다.
6. 직접 맵핑(Direct Mapping) : 주 기억 장치 블록들이 지정 된 하나의 캐시 라인으로만 사상 가능한 맵핑, 구현이 간단, 비용 절약, 적중율 하락
7. 어소시에이티브 맵핑(Associative Mapping) : 직접 매핑 방식의 단점을 보완한 방식으로 모든 태그를 병렬로 검사하여 적중율은 높지만, 복잡하고, 비용이 높아 거의 사용하지않음
8. 세트- 어소시에이티브 맵핑(Set-Associative Mapping) : 직접 과 어소시에이티브 맵핑을 장점들만 취한 방식 같은 인덱스 주소에 두개 이상의 tag를 저장한다.
