...
> 관계형 데이터모델을 지양, 데이터를 저장, 조회하는데 특화, 스키마 없이 또는 느슨한 스키마를 제공하는 저장소.  
> NoSQL의 종류에 따라 쓰기/읽기 특화, 2차 인덱스 지원, 오토 샤딩 지원 같은 고유한 특징이 있다.  
> 대량의 데이터를 빠르게 처리하기 위해서 메모리를 임시 저장소로 사용한다거나, 동적인 스케일 아웃을 제공하고, 가용성을 위해 데이터 복제등 관계형 데이터 베이스에서 제공하지 못하는 성능과 특징을 가지고 있다.

1. CAP이론 : 일관성(Consistency), 가용성(Availability), 네트워크 분할 허용성(Partition Tolerance), 3가지를 말하며 분산 시스템에서는 세가지 속성을 모두 만족하는 부분이 존재하지 않는다.
2. 일관성 : 동시성 또는 동일성, 다중 클라이언트가 같은 시간에 조회하는 데이터의 동일성을 보증 하는 것, NoSQL은 RDBMS와 다르게 느슨하게 처리되어 동일한 데이터가 나타나지 않을 수 있다. 느슨함 => 데이터 변경이 시간의 흐름에 따라 노드에 전파 되는 것, 최종적 일관성, 또는 궁극적 일관성을 지원한다고 표현한다.
3. NoSQL 분산 노드간 데이터 동기화 기법 : 1. 데이터 저장 결과를 응답전 모든 노드의 데이터를 저장하는 동기식으로 응답 속도를 희생해서 데이터 정합성을 보장한다. 2. 메모리나 임시 파일에 기록하고 먼저 응답, 이벤트와 프로세스를 사용하여 노드로 데이터를 동기화하는 비동기식 방법이 있다. 빠른 응답 시간을 보장 쓰기 노드 장애시 데이터 손실 위험이 있다.
4. 가용성 : 모든 클라이언트의 읽기와 쓰기 요청에 대하여 항상 응답 가능해야 함을 보증하는 것 또 다른 말로는 내고장성이라 지칭한다. 가용성이 보장된 NoSQL은 클러스터 내에서 몇개의 노드가 망가지더라도 정상적인 서비스를 제공가능하다. 가용성 보장을 위해 데이터 복제(Replication)을 사용하기도 한다. 데이터 중복 저장 기법은 동일한 데이터 저장소를 하나 더 만드는 Master-Slave(Main-Sub), 데이터 단위 중복 저장 Peer-to-Peer기법이 존재한다.
5. 네트워크 분할 허용성 : 지역적으로 분할된 네트워크 환경에서 동작하는 시스템에서 두 지역 간의 네트워크가 단절 되거나 네트워크 데이터의 유실이 일어나더라도 각 지역 내의 시스템은 정상적으로 동작하는 것을 말한다. 즉 각 네트워크는 서로 간의 장애에 영향을 받지 않아야 한다는 것.
6. PACELC 이론은 CAP 이론으로 부족한 부분을 보완하기위해 네트워크 장애 상황과 정상 상황으로 나누어서 설명하는 이론이다. P(네트워크 파티션)상황에서 A(가용성)과 C(일관성)의 상충 관계와 E(else, 정상)상황에서 L(지연 시간)과 C(일관성)의 상충 관계를 설명한다.

### 저장방식에 따른 NoSQL 분류

1. Key-Value Model, Document Model, Column Model, Graph Model로 분류할 수 있다.
2. Key-Value Model : 가장 기본적인 형태의 NoSQL, 키 하나로 데이터 하나를 저장 및 조회 가능하다. 단일 키-값 구조를 가지고, 단순한 구조로 복잡한 조회 연산을 지원하지 않는다. 보통 고속 읽기와 쓰기에 최적화, 사용자의 프로필 정보, 웹서버 클러스터 세션, 장바구니, URL단축 등 정보 저장 등에 사용되고, 트랜잭션 처리가 불가능하여 데이터 정합성을 보장 할 수 없다. 예 : Redis
3. Document Model : 하나의 키-문서 저장하고 조회한다. 논리적인 데이터 저장과 조회가 RDBMS와 유사하다. 키는 문서에 대한 ID로 표현, 저장 된 문서를 컬렉션으로 관리하며 저장과 동시에 ID에 대한 인덱싱을 한다.(커버링 인덱스가 가능 O(1) 조회) 보통 문서형 NoSQL B트리 인덱스를 사용하여 2차 인덱스를 생성,B 트리는 크기가 커지면 커질수록 새로운 데이터를 입력하거나 삭제할 때 성능이 떨어지게 된다. 그렇기 때문에 읽기와 쓰기의 비율이 7:3 정도일 때 가장 좋은 성능을 보인다. 중앙 집중식 로그 저장, 타임라인 저장, 통계 정보 저장 등에 사용된다. 예 : MongoDB
4. Column Model : 하나의 키-N * 컬럼Name&컬럼value구조로 데이터 저장, 조회한다. 모든 컬럼은 항상 타임 스템프 값이 같이 저장된다, 구글의 빅테이블이 대표한다. Row Key, Column Key, Column Family등 공통 개념이 존재하고 저장 기본 단위는 컬럼 : 컬럼 이름, 값, 타임스템프로 구성된다.
컬럼의 집합을 Row로 지칭하며, Row key는 유일한 식별 값이며 로우들의 집합은 key Space가 된다. 읽기에 특화된 NoSql로 데이터를 먼저 커밋로그와 메모리에 저장 후 응답하기에 빠른 응답 속도를 제공, 읽기 대비 쓰기 연산이 많은 서비스나 대량의 데이터를 입력하고 조회하는 서비스에 적합하다. 채팅 내용 저장, 실시간 분석을 위한 데이터 저장소등에 쓰인다. 예 : 빅테이블
5. Graph Model : 레이블(Lable), 노드, 속성과 값, 순회(Traversal), 스키마가 고정 불변이 아닌 데이터가 늘어남에 따라 지속적으로 갱신되는 형태의 모델 노드는 키-값을 가지고 있으며 하나 이상의 레이블을 가질 수 있다. 수백만, 수억개의 데이터들의 관계를 다루는데 적합한 데이터모델이다. 기존 RDBMS보다 좀 더 느슨한 스키마를 가진 관계형 데이터 베이스 예 : Grape DB